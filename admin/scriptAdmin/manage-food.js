...
function openDeleteCategoryModal(categoryName) {
  ...
  document.getElementById("categoryPageName").textContent = categoryName
    .toLowerCase()
    .replace(/[^a-zA-Z0-9]/g, "");
...
function closeDeleteConfirm() {
  const m = document.querySelector(".modal:last-of-type");
  if (m) {
    m.remove();
    document.body.style.overflow = "auto";
  }
...
document.addEventListener("DOMContentLoaded", function () {
  document
    .getElementById("image_upload_radio")
    .addEventListener("change", toggleImageInput);
  document
    .getElementById("image_url_radio")
    .addEventListener("change", toggleImageInput);
  document
  ...
  document
    ...
    .addEventListener("input", function (e) {
      ...
      if (v === "confirm") {
        ...
        btn.disabled = true;
        if (v.length > 0) e.target.classList.add("invalid");
      }
    });
...
function filterFoodItems() {
  ...
  visibleItems.forEach((item) => {
    const section = item.element.closest(".category-section");
...
function clearFilters() {
  ...
  updateResultsCount(document.querySelectorAll(".food-item").length);
}
function updateResultsCount(count) {
  const d = document.getElementById("resultsCount");
  if (d) d.textContent = `Showing ${count} food item(s)`;
}
...
function showNotification(message, type = "info", duration = 4000) {
  ...
  }</div><div class="notification-text">${message}</div></div><button class="notification-close" onclick="closeNotification(this)"><i class="fas fa-times"></i></button></div><div class="notification-progress"></div>`;
...
function saveFilterState() {
  ...
  localStorage.setItem("foodFilterState", JSON.stringify(s));
}
function loadFilterState() {
  const saved = localStorage.getItem("foodFilterState");
  if (!saved) return;
  const s = JSON.parse(saved);
  document.getElementById("searchInput").value = s.search || "";
  document.getElementById("categoryFilter").value = s.category || "";
  document.getElementById("minPrice").value = s.minPrice || "";
  document.getElementById("maxPrice").value = s.maxPrice || "";
  document.getElementById("sortBy").value = s.sortBy || "name-asc";
  filterFoodItems();
}
function setupFilterStateSaving() {
  ["searchInput", "categoryFilter", "minPrice", "maxPrice", "sortBy"].forEach(
    (id) => {
      const el = document.getElementById(id);
      if (el) {
        el.addEventListener("change", saveFilterState);
        el.addEventListener("keyup", saveFilterState);
      }
    }
  );
}
