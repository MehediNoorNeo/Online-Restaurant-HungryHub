...
document.getElementById("areaForm").addEventListener("submit", function (e) {
  ...
  if (areaName.length < 2) {
    e.preventDefault();
    showNotification("Area name must be at least 2 characters long", "error");
    return;
  }
});
...
function showNotification(message, type = "info", duration = 4000) {
  ...
  n.innerHTML = `
        <div class="notification-content">
            ...
            <div class="notification-message"><div class="notification-title">${
              type.charAt(0).toUpperCase() + type.slice(1)
            }</div><div class="notification-text">${message}</div></div>
